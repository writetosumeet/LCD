...
// RS - PC0 / EN - PC1 / D7 - PC5 / D6 - PC4 / D5 - PC3 / D4 - PC2 
...
void byteToLCD(unsigned char flag, unsigned char byteToWrite)
{
  ...
  PORTC = (PORTC & 0b11001111) | ((byteToWrite & 0b11000000)>>2); // mask not to be changed bits / shift and align byteToWrite with PORTC pins
  PORTC = (PORTC & 0b11110011) | ((byteToWrite & 0b00110000)>>2); // mask not to be changed bits / shift and align byteToWrite with PORTC pins
  
  // RS = 0 for command / 1 for data
  if (flag == 0)
    PORTC = (PORTC & 0b11111110) | (0b00000000);
  else
    PORTC = (PORTC & 0b11111110) | (0b00000001);
  
  // Enable = 1
  PORTC = (PORTC & 0b11111101) | (0b00000010);
  
  // wait 100 micro second
  _delay_us(100);
  
  // Enable = 0
  PORTC = (PORTC & 0b11111101) | (0b00000000);
  
  // wait 100 micro second
  _delay_us(100);
  
  // write low bits on selected Ports
  PORTC = (PORTC & 0b11001111) | ((byteToWrite & 0b00001100)<<2); // mask not to be changed bits / shift and align byteToWrite with PORTC pins
  PORTC = (PORTC & 0b11110011) | ((byteToWrite & 0b00000011)<<2); // mask not to be changed bits / shift and align byteToWrite with PORTC pins
...
/*int main (void)
{
  ...
  // Write to LCD
  writeToLCD("Strange power issue 4 bit 11 june");
